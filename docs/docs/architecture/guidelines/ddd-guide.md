...
- 複数コンテキスト間を横断するユースケース（例: 掲示場と検証の連携）は、アプリケーション層で調停し、ドメイン層の直接参照を避ける。
...
| ----------------------- | ---------------------------- | ---------------------------------- |
| `features/board`        | 掲示場管理コンテキスト       | 掲示場番号、位置、掲示状態の管理   |
| `features/verification` | 検証コンテキスト             | 掲示場データの検証依頼、承認フロー |
...
| ----------------------------------- | -------------------------------------------------------- | ---------------------------------------------------------- | -------------------------------------------------------------- |
| 検証依頼（Verification Request）    | サポーターへ掲示場の現地確認を依頼するドキュメント。     | `features/verification` のユースケースで生成・受付。       | 承認ステータスの遷移条件となり、未対応の場合は公開停止を判断。 |
...
2. **ドメインシナリオ分析**: 実際の業務フロー（例: 掲示場設置→検証→公開）を時系列で整理し、イベント単位でストーリーを作る。
3. **集約の抽出**: 不変条件を満たすために一括更新すべきデータのまとまりを抽出し、集約ルートを仮決定する。
4. **値オブジェクト化**: 単純な型では不十分な概念（例: 緯度経度、掲示場番号形式）を値オブジェクトとして定義する。
...
- 不変条件（例: 掲示場番号は自治体内でユニーク）は集約ルート内で検証し、アプリケーション層に委ねない。
...
```typescript title="例: 掲示場距離判定のドメインサービス"
