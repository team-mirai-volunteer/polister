...
**Polister** (Politics + Poster) - 選挙ポスター掲示場位置管理サービス

### 1.2 プロジェクトの目的

選挙時のポスター掲示場の位置情報をデジタル化し、オープンデータとして提供することで、選挙活動の効率化と透明性の向上を図る。

**背景**:
チームみらいは参議院選挙において、全国の自治体から提供された掲示場情報を手作業でCSV化・正規化した実績がある。この経験から、統一的なプラットフォームの必要性と、データ正規化のノウハウを蓄積している。

### 1.3 背景・課題

- **課題1：情報の分散と非標準化**
  - 自治体ごとに提供される掲示場情報の形式が異なる（デジタルデータ、紙の地図、口頭など）
  - 統一的なデータベースが存在せず、候補者や支援者が個別に情報収集する必要がある

- **課題2：情報の正確性と最新性**
  - 提供された情報が古かったり、現地の実態と異なることがある
  - 掲示場の位置変更や撤去の情報が反映されていない
...
- **課題4：国政選挙における事前準備の困難さ**
  - 国政選挙の掲示場情報は選挙公示後に提供されることが多く、事前準備が難しい
  - 地方選挙と国政選挙で同じ掲示場が使われるケースがあり、地方選挙中のデータ収集が有効
  - 過去の記憶や目撃情報など、非公式な情報源の活用が必要

### 1.4 解決方針

- **方針1：統一的なデジタルプラットフォーム**
  - 全国の選挙ポスター掲示場情報を一元管理するWebサービスを構築
  - 地図上で視覚的に掲示場の位置を確認できるインターフェース

- **方針2：クラウドソーシングによるデータ収集と検証**
  - サポーターや協力者が現地確認を行い、データの正確性を検証
  - 地方選挙中に国政選挙用の掲示場情報を収集（スマホ撮影、位置情報記録）
...
| 用語                                | 定義                                                                     | 補足                                                         |
| ----------------------------------- | ------------------------------------------------------------------------ | ------------------------------------------------------------ |
| 掲示場（Board）                     | 自治体が設置する選挙ポスター掲示場。掲示場番号、所在地、設置状況を持つ。 | `features/board` コンテキストの集約ルート候補。              |
| 検証依頼（Verification Request）    | サポーターに対して掲示場の現地確認を求める依頼。                         | `features/verification` コンテキストのユースケースで生成。   |
...
- **地図表示機能（Mapbox GL JS）**
  - 地図上にポスター掲示場の位置をマーカーで表示
  - 地図のズーム、パン操作
  - 衛星写真・地図の切り替え
  - **カスタム地図スタイル**:
    - ポスター掲示場の視認性を最優先にした配色設計
    - 道路、建物、公共施設などのランドマークを明確に表示
    - 掲示場マーカーが地図上で目立つような配色とデザイン
...
- **データインポート**
  ...
  - **KML形式でのデータ一括登録**
    ...
    - GeoJSONへの変換対応
  - **GeoJSON形式でのデータ一括登録**
    - FeatureCollection（Point、Polygon）に対応し、複数掲示場をまとめて取り込み
    - 座標系はWGS84（EPSG:4326）を必須とし、異なる場合は警告と再変換フローを提供
    - Featureのpropertiesから掲示場番号・住所・信頼度などの属性マッピングを定義済みテンプレートとカスタム設定で選択可能
...
- **オープンデータ公開**
  - CSV、GeoJSON形式でのダウンロード（GeoJSONはFeatureCollection形式／EPSG:4326で提供）
  - REST API による データアクセス（`format=geojson` クエリでGeoJSONレスポンスを返却）
...
1. **地方選挙中の収集**:
   - 協力者が地方選挙中に掲示場を発見・撮影
   - スマホアプリから位置情報付きで報告
   - 「ここで国政選挙の掲示場を見た」という情報を記録

2. **過去情報の活用**:
   - 「以前この場所に掲示場があった」という記憶ベースの情報
...
- **フロントエンド**
  ...
  - Mapbox GL JS（地図表示）
    - カスタム地図スタイルの提供（ポスター掲示場視認性に最適化）
    - 道路や建物などのランドマークを強調表示
    - 掲示場マーカーの視認性を考慮した配色設計
