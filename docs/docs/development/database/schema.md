...
Polisterのデータベーススキーマは、PostgreSQL 17 + PostGIS 3.5を使用し、掲示場の位置情報を空間データとして管理します。
...
掲示場の位置情報と基本データを管理します。

| カラム名          | 型               | 説明                                     |
| ----------------- | ---------------- | ---------------------------------------- |
| id                | UUID             | 主キー                                   |
| board_number      | Text             | 掲示場番号（例: `01-2`）。空の場合はNULL |
...
### board_histories（掲示場変更履歴）

**Phase 1補足で追加予定**

掲示場情報の変更履歴を記録し、監査証跡を提供します。
...
4. **変更履歴の追跡不能**: 誰が掲示場情報を変更したかが不明になる
...
1. **削除前の状態を追跡**: 掲示場がいつ削除されたか、なぜ削除されたかを記録
2. **誤削除のロールバック**: 必要に応じて削除前の状態に復元可能
3. **データ品質分析**: 削除された掲示場の傾向を分析（撤去された掲示場の統計など）
