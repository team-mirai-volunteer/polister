# 掲示場データのエクスポート／インポート手順

本ドキュメントは、自治体・掲示場・掲示場履歴のデータを既存DBからエクスポートし、別環境へインポートするためのシンプルな手順をまとめています。
...
`scripts/poster-board-import/export.sh` を使うと、自治体／掲示場／掲示場履歴のCSVをまとめて出力できます。引数に出力先ディレクトリを指定します（省略時は `exports/`）。
...
- `merge_boards.sql` では `deleted_at IS NULL` な掲示場のみ取り込みます。座標や自治体名が一致しないレコードがある場合は、ステージングテーブルを確認して補正してください。掲示場番号は `01-2` のような文字列を想定しており、空文字の場合は `NULL` として扱われます。
